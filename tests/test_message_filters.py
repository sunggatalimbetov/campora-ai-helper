...
class MentionExtractionTests(unittest.TestCase):
    def test_extracts_query_when_bot_is_mentioned(self):
        text = "@vectir_bot when is the application deadline?"
        entities = [MessageEntity(type=MessageEntity.MENTION, offset=0, length=11)]

        query = extract_mentioned_query(text, entities, "vectir_bot")
    ...
    def test_strips_multiple_mentions_and_keeps_remaining_text(self):
        text = "@vectir_bot please help with the deadline @vectir_bot"
        entities = [
            MessageEntity(type=MessageEntity.MENTION, offset=0, length=11),
            MessageEntity(type=MessageEntity.MENTION, offset=42, length=11),
        ]
