-- Update all three search RPCs to accept an array of chat IDs instead of a single BIGINT.
-- DROP the old single-BIGINT overloads first to avoid PostgREST ambiguity errors.
-- Run after 009_universities_and_chats.sql.

DROP FUNCTION IF EXISTS public.match_messages(vector, integer, bigint);
DROP FUNCTION IF EXISTS public.hybrid_search(text, vector, integer, double precision, double precision, integer, bigint);
DROP FUNCTION IF EXISTS public.match_messages_and_questions(vector, integer, bigint);

CREATE OR REPLACE FUNCTION public.match_messages(
    query_embedding vector(1536),
    match_count int DEFAULT 5,
    filter_chat_ids BIGINT[] DEFAULT NULL
)
RETURNS TABLE (
    id BIGINT,
    chat_id BIGINT,
...
BEGIN
    ...
    WHERE m.embedding IS NOT NULL
      AND (filter_chat_ids IS NULL OR m.chat_id = ANY(filter_chat_ids))
